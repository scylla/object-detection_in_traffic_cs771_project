...
def getLabel(image_path):
	...
	if pred == 1:
		return (1, clf.decision_function(fd))
	else :
		return (0, clf.decision_function(fd))	
...
while True:
	...
	for c in cnts:
		...
		if output == 1 and label_dis > 0.2:
			car_count += 1
			cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
			cv2.putText(frame, "Car " + str(label_dis), (x - 3 ,y - 3), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (0, 255, 0),2)
